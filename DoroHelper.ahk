...
EventLarge() {
    ...
    if g_settings["EventLargeStory"] {
        ...
        else while (ok := FindText(&X := "wait", &Y := 1, NikkeX + 0.348 * NikkeW . " ", NikkeY + 0.751 * NikkeH . " ", NikkeX + 0.348 * NikkeW + 0.092 * NikkeW . " ", NikkeY + 0.751 * NikkeH + 0.042 * NikkeH . " ", 0.4 * PicTolerance, 0.4 * PicTolerance, FindText().PicLib("大活动·STORY"), , , , , , , TrueRatio, TrueRatio)) {
            AddLog("尝试进入对应活动页")
            FindText().Click(X - 50 * TrueRatio, Y, "L")
            Sleep 500
        }
...
AwardPass() {
    ...
    while true {
        ...
        ; 检测是否有其他未完成的通行证
        else if (ok := FindText(&X, &Y, NikkeX + 0.890 * NikkeW . " ", NikkeY + 0.149 * NikkeH . " ", NikkeX + 0.890 * NikkeW + 0.010 * NikkeW . " ", NikkeY + 0.149 * NikkeH + 0.016 * NikkeH . " ", 0.4 * PicTolerance, 0.4 * PicTolerance, FindText().PicLib("红点"), , , , , , , TrueRatio * 0.8, TrueRatio * 0.8)) {
            FindText().Click(X, Y, "L")
            BackToHall()
            continue
        }
