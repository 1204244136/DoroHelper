        ...
        if groupData.Has(Hashed) {
            ...
            if IsObject(memberInfo) && memberInfo.Has("expiry_date") && memberInfo.Has("tier") {
                ...
                if (expiryDate >= CurrentDate) {
                    ...
                    isMember := true ; 找到匹配项，设置标志位
                    break ; 找到匹配项后立即退出循环
                } else {
                    ; 有效期已过
                    AddLog("会员已过期 (到期日: " expiryDate ")。已降级为普通用户")
                    isMember := false
                    ; 考虑到可能有多块硬盘，即使一块过期，也不能立即返回
                    ; 而是继续检查其他硬盘哈希
                }
            } else {
...
CalculateSponsorInfo(thisGuiButton, info) {
    ...
    ; 给出提示
    MsgBox("赞助信息已生成并复制到剪贴板，请将其连同付款记录发给我。`n可以加入DoroHelper反馈群(584275905)并私信我`n也可以发我的 qq 邮箱(1204244136@qq.com)（只选一个即可，邮箱标题建议注明X个月的XDoro）`n24 小时内我会进行登记并通知，之后重启软件并勾选用户组的「自动检查」即可")
}
;tag 帮助
ClickOnHelp(*) {
    MyHelp := Gui(, "帮助")
